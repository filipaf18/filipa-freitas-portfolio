...
export default async function () {
  ...
  try {
    ...
    assert(modal.includes("Filipa") && modal.includes("Vitória") && modal.includes("cerca de 19 pedidos"), "a confirmação diz quem e quantos pedidos: " + modal);
    ...
    eq(prompts.filter((x) => x.includes("escreve a EMENTA")).length, 2, "ementa em família em dois blocos");
    ...
    eq(prompts.length, 23, "19 pedidos previstos + 4 das metades");
