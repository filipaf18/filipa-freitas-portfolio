...
export default async function () {
  const JSONF = `
    ...
    if (m) { window.__k.b2++; if (window.__k.b2 <= 2) throw { code: "upstream_error", message: "ligação caiu" };
  ...
  const { page } = app;
  try {
    await app.go("plano"); await page.click("#genPlan"); await page.waitForTimeout(9000);
    ...
    eq(k.b2, 2, "o segundo bloco falhou duas vezes (pedido + repetição) e parou");
    ...
    eq(k.b2, 3, "só o segundo bloco foi pedido");
