(() => {
  ...
  const S = {
    ...
    prefs: [],             // gostei/não gostei por refeição [{id, at, dia, refeicao, itens, voto}]; voto 0 = nota livre
    partialFor: null,      // refeição com a lista "comi parte" aberta
  ...
  function subscribeProfile() {
    ...
    S.unsub.push(S.db.doc(`adherence/${S.pid}`).onSnapshot((snap) => {
      ...
      if (S.sample) estimateOthers().catch((e) => console.warn("estimar", e));
  ...
  async function setAdherence(mealName, status, texto, itens) {
    const day = { ...todayAdherence() };
    if (status === null) delete day[mealName]; else day[mealName] = { status, ...(texto ? { texto } : {}), ...(Array.isArray(itens) ? { itens } : {}), at: new Date().toISOString() };
    S.adherence = { ...S.adherence, [localDate()]: day };
    await saveAdherence();
    if (status === "outro" && texto) estimateOthers().catch((e) => console.warn("estimar", e));
  }
  /**
   * "Comi outra coisa" com texto: um pedido rápido estima os macros do que a pessoa escreveu, para
   * Hoje contar com isso. Fica gravado na adesão (kcal, proteína…, estimado: true); uma só tentativa por registo.
   */
  async function estimateOthers() {
    if (!S.sample || S.estimating) return false;
    const date = localDate(); const day = todayAdherence();
    const pend = Object.entries(day).filter(([, a]) => a.status === "outro" && a.texto && a.kcal == null && !a.est_at);
    if (!pend.length) return false;
    S.estimating = true;
    try {
      for (const [nome, a] of pend) {
        let est = null;
        try {
          est = await S.sample.json(`Estima os macros do que esta pessoa comeu numa refeição (${nome}), a partir do que escreveu, em português de Portugal. Usa porções médias portuguesas quando não houver quantidade; num intervalo ("100-120 g") usa o meio. Ignora frases que não sejam comida.\nTEXTO: ${JSON.stringify(a.texto)}\nResponde APENAS com JSON: {"itens":[{"alimento":"…","quantidade":"60 g"}],"kcal":480,"proteina_g":48,"hidratos_g":50,"gordura_g":14,"fibra_g":4}`, { cache: false, modelTier: "quick" });
        } catch (e) { if (e?.code === "cancelled" || e?.code === "not_granted" || e?.code === "rate_limited") throw e; S.diag.lastErr = `estimar "${nome}": ${e?.code || e?.message}`; renderDiag(); }
        const cur = (S.adherence[date] || {})[nome]; if (!cur || cur.status !== "outro" || cur.texto !== a.texto) continue;
        const upd = { ...cur, est_at: new Date().toISOString() };
        if (est && Number.isFinite(Number(est.kcal))) Object.assign(upd, { kcal: Math.round(Number(est.kcal)), proteina_g: Math.round(Number(est.proteina_g) || 0), hidratos_g: Math.round(Number(est.hidratos_g) || 0), gordura_g: Math.round(Number(est.gordura_g) || 0), fibra_g: Math.round((Number(est.fibra_g) || 0) * 10) / 10, itens_estimados: (Array.isArray(est.itens) ? est.itens : []).map((i) => `${i.alimento || ""} ${i.quantidade || ""}`.trim()).filter(Boolean).slice(0, 12), estimado: true });
        S.adherence = { ...S.adherence, [date]: { ...(S.adherence[date] || {}), [nome]: upd } };
      }
      await saveAdherence();
    } finally { S.estimating = false; }
    return true;
  }
  ...
  function adherenceButtons(m, compact) {
    ...
    return `${b("comi", "Comi ✓")}${b("parcial", "Comi parte")}${b("outro", "Comi outra coisa")}${b("saltei", "Saltei")}`;
  }
  /** Lista de itens para marcar o que se comeu de uma refeição. */
  function partialHtml(m) {
    const cur = todayAdherence()[m.nome]; const chosen = new Set(cur?.status === "parcial" && Array.isArray(cur.itens) ? cur.itens.map(norm) : []);
    return `<div class="partial" data-partial="${esc(m.nome)}"><p class="small muted">O que comeste desta refeição?</p>${(m.itens || []).map((i, k) => `<label><input type="checkbox" data-pitem="${k}" ${chosen.size === 0 || chosen.has(norm(i.alimento)) ? "checked" : ""}> ${esc(i.alimento)} <span class="muted">${esc(i.quantidade)}</span></label>`).join("")}<div class="row" style="justify-content:flex-end; gap:6px"><button type="button" class="btn ghost sm" data-pcancel>Cancelar</button><button type="button" class="btn sm primary" data-psave="${esc(m.nome)}">Guardar</button></div></div>`;
  }
  /** Macros do que já entrou hoje: refeições comidas, comidas em parte e extras. */
  /** O que entrou de facto numa refeição com adesão registada (comi, parte, outra coisa estimada, saltei); null sem registo ou sem estimativa. */
  function consumedMacros(m, a) {
    if (!a) return null;
    if (a.status === "comi") return mealMacros(m);
    if (a.status === "parcial") { const set = new Set((a.itens || []).map(norm)); return mealMacros({ ...m, itens: (m.itens || []).filter((i) => set.has(norm(i.alimento))) }); }
    if (a.status === "outro") return a.kcal != null ? { kcal: Number(a.kcal) || 0, proteina_g: Number(a.proteina_g) || 0, hidratos_g: Number(a.hidratos_g) || 0, gordura_g: Number(a.gordura_g) || 0, fibra_g: Number(a.fibra_g) || 0 } : null;
    if (a.status === "saltei") return { kcal: 0, proteina_g: 0, hidratos_g: 0, gordura_g: 0, fibra_g: 0 };
    return null;
  }
  function eatenToday() {
    const adh = todayAdherence(); const tot = { kcal: 0, proteina_g: 0, hidratos_g: 0, gordura_g: 0, fibra_g: 0 }; const planned = { ...tot };
    const add = (t, mm) => { t.kcal += mm.kcal; t.proteina_g += mm.proteina_g; t.hidratos_g += mm.hidratos_g; t.gordura_g += mm.gordura_g; t.fibra_g += mm.fibra_g; };
    let desconhecidas = 0;
    for (const m of todayMeals()) {
      const a = adh[m.nome]; const full = mealMacros(m);
      if (!a) { add(planned, full); continue; }
      const c = consumedMacros(m, a);
      if (c) add(tot, c); else if (a.status === "outro") desconhecidas++;
    }
    for (const x of (S.plan?.extras || []).filter((x) => x.date === localDate())) { tot.kcal += x.kcal || 0; tot.proteina_g += x.proteina_g || 0; }
    return { comido: tot, por_vir: planned, desconhecidas };
  }
  function renderEaten() {
    const el = $("eatenChart"); if (!el) return;
    if (!S.plan?.plan) { el.innerHTML = ""; return; }
    const { comido, por_vir, desconhecidas } = eatenToday();
    const metas = S.plan.plan.metas_diarias || {}; const t = planTargets(profile()) || {};
    const alvo = { kcal: metas.kcal || t.energia_alvo_kcal || 0, proteina_g: metas.proteina_g || t.proteina_alvo_g_dia || 0, hidratos_g: metas.hidratos_g || 0, gordura_g: metas.gordura_g || 0, fibra_g: metas.fibra_g || 25 };
    const rows = [["kcal", "Energia", "kcal"], ["proteina_g", "Proteína", "g"], ["hidratos_g", "Hidratos", "g"], ["gordura_g", "Gordura", "g"], ["fibra_g", "Fibra", "g"]].filter(([k]) => alvo[k]);
    el.innerHTML = `<div class="row between"><span class="eyebrow">Já comeste hoje</span><span class="small muted">${desconhecidas ? `${desconhecidas} refeição${desconhecidas > 1 ? "ões" : ""} "outra coisa" sem conta` : "contado pela tabela de alimentos"}</span></div>
      <div class="meters">${rows.map(([k, label, un]) => { const v = Math.round(comido[k]), a = Math.round(alvo[k]), pv = Math.round(comido[k] + por_vir[k]); const pct = Math.min(100, Math.round(v / a * 100)); const pp = Math.min(100, Math.round(pv / a * 100)); return `<div class="meter" title="${esc(label)}: ${v} de ${a} ${un}; com o resto do plano ${pv}"><div class="row between"><span class="k">${esc(label)}</span><span class="num v"><strong>${v}</strong> <span class="muted">/ ${a} ${un}</span></span></div><div class="track"><div class="fill" style="width:${pct}%"></div><div class="tick" style="left:${pp}%"></div></div></div>`; }).join("")}</div>
      ${(() => { const falta = Math.round(alvo.kcal - comido.kcal), fp = Math.round(alvo.proteina_g - comido.proteina_g), resto = Math.round(por_vir.kcal); if (!alvo.kcal) return ""; return `<p class="small" id="eatenLeft">${falta > 0 ? `Faltam <strong class="num">${falta} kcal</strong>${fp > 0 ? ` e <strong class="num">${fp} g</strong> de proteína` : ""} para a meta de hoje` : `Já passaste a meta de hoje em <strong class="num">${-falta} kcal</strong>`}${resto ? ` · o resto do plano de hoje traz ${resto} kcal` : ""}.</p>`; })()}
      <p class="hint">A barra é o que já entrou; o traço é onde ficas se seguires o resto do plano de hoje.</p>
      ${(() => { const adh = todayAdherence(); const desvio = Object.values(adh).some((a) => ["outro", "parcial", "saltei"].includes(a.status)) || (S.plan.extras || []).some((x) => x.date === localDate()); const porComer = todayMeals().some((m) => !adh[m.nome]); return desvio && porComer && S.sample ? `<div class="row"><button type="button" class="btn sm" id="rebalanceToday">🔁 Ajustar o resto do dia ao que já comi</button></div><p class="small muted" id="eatenNote"></p>` : ""; })()}`;
  }
  /** Botão em Hoje: as refeições por comer ajustam-se ao que já entrou (outra coisa, parte, saltei, extras). */
  async function rebalanceTodayFromHome() {
    const b = $("rebalanceToday"); const note = $("eatenNote"); if (!b || b.disabled) return;
    b.disabled = true; if (note) note.textContent = "A ajustar o resto do dia…";
    try {
      const r = await rebalanceRestOfDay({ dia: todayDayName(), fixas: [] });
      renderHome();
      const n2 = $("eatenNote"); if (n2) n2.textContent = r?.mudadas?.length ? `Ajustado: ${r.mudadas.join("; ")}. Dia: ${r.totais.kcal} kcal, ${r.totais.proteina_g} g de proteína.` : "Nada por ajustar.";
    } catch (e) {
      const n2 = $("eatenNote"); if (n2) n2.textContent = ERR_COPY[e?.code] || "Não foi possível ajustar agora. Tenta outra vez.";
      S.diag.lastErr = `ajustar hoje: ${e?.code || e?.message}`; renderDiag();
      const b2 = $("rebalanceToday"); if (b2) b2.disabled = false;
    }
  }
  ...
  function renderHome() {
    ...
    if (hasPlan) {
      if (!nm) { $("nextMealLabel").textContent = "Hoje"; $("nextMealBody").innerHTML = `<p class="muted small">O plano não tem refeições para hoje.</p>`; $("nextMealActions").innerHTML = ""; }
        ...
        $("nextMealActions").innerHTML = adherenceButtons(m) + `<button type="button" class="btn sm" data-recipe="${esc(todayDayName())}|${(S.plan.plan.dias.find((x) => x.dia === todayDayName())?.refeicoes || []).indexOf(m)}">🍳 Receita</button>`;
        const partWrap = $("nextMealPartial"); if (partWrap) partWrap.innerHTML = S.partialFor === m.nome ? partialHtml(m) : "";
      }
    ...
    const n = Object.values(adh); const c = n.filter((x) => x.status === "comi").length, pa = n.filter((x) => x.status === "parcial").length, o = n.filter((x) => x.status === "outro").length, sk = n.filter((x) => x.status === "saltei").length;
    $("adherenceSummary").textContent = meals.length ? `${c} seguida${c === 1 ? "" : "s"}${pa ? `, ${pa} em parte` : ""}${o ? `, ${o} diferente${o === 1 ? "" : "s"}` : ""}${sk ? `, ${sk} saltada${sk === 1 ? "" : "s"}` : ""} de ${meals.length}` : "";
    renderEaten();
  ...
  function weekStats(n = 7) {
    ...
    let parcial = 0;
    Object.entries(S.adherence).forEach(([d, meals]) => { if (!inWeek(d)) return; Object.values(meals).forEach((v) => { if (v.status === "comi") comi++; else if (v.status === "parcial") parcial++; else if (v.status === "outro") outro++; else if (v.status === "saltei") saltei++; diasComRegisto.add(d); }); });
    const totalAdh = comi + parcial + outro + saltei;
    // água
    ...
    return {
      ...
      adesao: { refeicoes_registadas: totalAdh, seguidas: comi, em_parte: parcial, diferentes: outro, saltadas: saltei, percentagem_seguida: totalAdh ? Math.round((comi + 0.5 * parcial) / totalAdh * 100) : null, dias_com_registo: diasComRegisto.size },
...
{"resumo":"3 a 5 frases","correu_bem":["..."],"ajustar":["..."],"propostas":[{"o_que":"ajuste concreto ao plano","porque":"1 frase"}],"para_o_medico":"frase curta ou null"}`;
  ...
  function renderRules() {
    ...
    pl.innerHTML = rows.length === 0
      ...
      : rows.map((p) => `<li><span>${p.voto === 1 ? "👍" : p.voto === -1 ? "👎" : "💬"} ${p.voto === 0 ? `<strong>${esc(p.refeicao)}${p.dia ? ` (${p.dia})` : ""}:</strong> ${esc(p.nota || "")}` : esc(p.dia ? `${p.refeicao} (${p.dia}): ${p.itens}` : p.itens)}${p.origem === "chat" ? ` <span class="meta">· do chat</span>` : ""}</span><button type="button" class="btn ghost sm" data-delpref="${p.id}">remover</button></li>`).join("");
  }
...
"preparacao_antecipada":["3 a 6 tarefas a fazer de uma vez, com o que rendem e quantos dias duram no frigorífico"]
  ...
  function prefOf(m) { const k = mealKey(m); return [...S.prefs].reverse().find((p) => p.key === k && p.voto !== 0) || null; }
  const notesOf = (m) => S.prefs.filter((p) => p.voto === 0 && p.key === mealKey(m));
  async function votePref(dayName, idx, voto) {
    ...
    S.prefs = S.prefs.filter((p) => !(p.key === key && p.voto !== 0));
...
"por_pessoa":[{"perfil":"${(fam.pessoas || [])[0]}","itens":[{"alimento":"...","quantidade":"130 g","estado":"cru","medida_caseira":"...","grupo":"proteina"}],"ordem":["legumes","proteína","hidratos"],"kcal":460,"proteina_g":38,"hidratos_g":42,"gordura_g":14,"fibra_g":8,"nota":""}]}}`;
  ...
  /** Soma as quantidades de um item para várias pessoas; devolve texto e a repartição. */
  function sumQuantities(parts) {
    let g = 0, un = 0, others = [];
    for (const { quantidade, food } of parts) { const q = parseQty(quantidade); const gr = gramsOf(quantidade, food); if (q && q.u === "un") un += q.n; else if (gr) g += gr; else if (quantidade) others.push(quantidade); }
    const bits = []; if (g) bits.push(`${Math.round(g / 5) * 5} g`); if (un) bits.push(`${fmtNum(un)} ${un === 1 ? "unidade" : "unidades"}`); bits.push(...others);
    return bits.join(" + ") || "q.b.";
  }
  /** Um alimento que esta pessoa não come (não gosta, alergia, intolerância), pelo nome ou pelo grupo. */
  function avoidsFood(id, alimento, grupo) {
    const q = S.profiles[id] || {};
    const skip = [...(q.dislikes || []), ...(q.allergies || []), ...(q.intolerances || [])].map(norm).filter(Boolean);
    const a = norm(alimento); const g = norm(grupo || "");
    return skip.some((d) => a.includes(d) || (d && g && d.startsWith(g.slice(0, 5))) || (/legumin/.test(d) && /grao|feijao|lentilha|ervilha|fava|soja|tremoco/.test(a)));
  }
  /**
   * A porção de uma refeição para outra pessoa: da refeição em família (se for), do plano dela (se
   * tiver o mesmo prato nesse dia) ou ajustada às metas dela a partir da porção de quem cozinha.
   */
  function portionFor(meal, dayName, id, fromId = S.pid) {
    if (id === fromId) return { itens: (meal.itens || []).map((i) => ({ ...i })), fonte: "mine" };
    const fd = (S.family?.dias || []).find((d) => norm(d.dia) === norm(dayName)); const slot = meal.familia ? famSlot(fd, meal.nome) : null;
    const pp = slot ? fd[slot]?.por_pessoa?.[id] : null;
    if (pp) return { itens: (pp.itens || []).map((i) => ({ ...i })), fonte: "familia", nota: pp.nota || "" };
    const deles = (S.plans?.[id]?.plan?.dias || []).find((d) => norm(d.dia) === norm(dayName))?.refeicoes?.find((m) => norm(m.nome) === norm(meal.nome));
    if (deles?.itens?.length) {
      const mine = new Set((meal.itens || []).map((i) => norm(i.alimento)));
      const comum = deles.itens.filter((i) => mine.has(norm(i.alimento))).length;
      if (comum * 2 >= Math.max(1, mine.size)) return { itens: deles.itens.map((i) => ({ ...i })), fonte: "plano" };
    }
    const tMine = planTargets(S.profiles[fromId] || {}, fromId) || {}; const tId = planTargets(S.profiles[id] || {}, id) || {};
    const rProt = (tId.proteina_alvo_g_dia || 80) / (tMine.proteina_alvo_g_dia || 80);
    const rKcal = tId.energia_alvo_kcal && tMine.energia_alvo_kcal ? tId.energia_alvo_kcal / tMine.energia_alvo_kcal : rProt;
    const clamp = (r) => Math.max(0.5, Math.min(2, r));
    const itens = (meal.itens || []).map((it) => {
      const f = findFood(it.alimento); const grupo = it.grupo || guessGroup(f);
      if (avoidsFood(id, it.alimento, grupo)) return null;
      const q = parseQty(it.quantidade); const ratio = clamp(grupo === "proteina" ? rProt : rKcal);
      let quantidade = it.quantidade;
      if (q && (q.u === "g" || q.u === "ml")) quantidade = fmtQty({ n: Math.max(5, Math.round(q.n * ratio / 5) * 5), u: q.u });
      else if (q && q.u === "un" && ratio !== 1) quantidade = fmtQty({ n: Math.max(0.5, Math.round(q.n * ratio * 2) / 2), u: "un" });
      return { ...it, quantidade, grupo };
    }).filter(Boolean);
    return { itens, fonte: "estimado", ratio: { proteina: clamp(rProt), energia: clamp(rKcal) } };
  }
  /**
   * A receita de uma refeição, para quem cozinha: escolhe-se para quem se cozinha (só para mim, ou
   * também para outros) e as quantidades somam-se pessoa a pessoa, cada uma com a sua porção.
   */
  function openRecipe(dayName, idx) {
    const day = S.plan?.plan?.dias?.find((d) => d.dia === dayName); const meal = day?.refeicoes?.[idx]; if (!meal) return;
    const fd = (S.family?.dias || []).find((d) => norm(d.dia) === norm(dayName)); const slot = meal.familia ? famSlot(fd, meal.nome) : null; const fm = slot ? fd[slot] : null;
    const members = [S.pid, ...PROFILE_IDS.filter((id) => id !== S.pid && hasProfile(id))];
    const inFam = (id) => !!fm?.por_pessoa?.[id];
    const wrap = document.createElement("div"); wrap.className = "modal";
    const build = () => {
      const chosen = [...wrap.querySelectorAll("[data-for]:checked")].map((c) => c.dataset.for);
      const portions = {}; chosen.forEach((id) => { portions[id] = portionFor(meal, dayName, id); });
      const byItem = new Map();
      chosen.forEach((id) => portions[id].itens.forEach((i) => { const k = norm(i.alimento); if (!byItem.has(k)) byItem.set(k, { alimento: i.alimento, medida: i.medida_caseira || "", parts: [] }); byItem.get(k).parts.push({ id, quantidade: i.quantidade, food: findFood(i.alimento) }); }));
      const rows = [...byItem.values()].map((it) => `<li><span>${esc(it.alimento)}${chosen.length === 1 && it.medida ? ` <span class="meta muted">(${esc(it.medida)})</span>` : ""}</span><span class="qty num"><strong>${esc(chosen.length === 1 ? (it.parts[0].quantidade || sumQuantities(it.parts)) : sumQuantities(it.parts))}</strong>${chosen.length > 1 ? `<span class="who">${it.parts.map((x) => `${esc(nameOf(x.id))} ${esc(x.quantidade)}`).join(" · ")}${it.parts.length < chosen.length ? ` · sem: ${chosen.filter((id) => !it.parts.some((x) => x.id === id)).map(nameOf).join(", ")}` : ""}</span>` : ""}</span></li>`).join("");
      $("rcRows").innerHTML = rows || `<li class="muted small">${chosen.length ? "Sem itens." : "Escolhe pelo menos uma pessoa."}</li>`;
      const fontes = chosen.filter((id) => id !== S.pid).map((id) => { const f = portions[id].fonte; return `${nameOf(id)}: ${f === "familia" ? "porção da refeição em família" : f === "plano" ? "porção do plano dela" : "porção ajustada às metas dela" + (portions[id].nota ? ` (${portions[id].nota})` : "")}`; });
      $("rcWho").textContent = chosen.length > 1 ? `Para ${chosen.length} pessoas, sem sobrar. ${fontes.join(" · ")}.` : chosen.length === 1 && chosen[0] !== S.pid ? `Só para ${nameOf(chosen[0])}. ${fontes.join("")}.` : "Só para ti.";
    };
    const prep = fm ? fm.preparacao : meal.preparacao;
    wrap.innerHTML = `<div class="box editor" role="dialog" aria-modal="true">
      <div class="row between"><h2>🍳 ${esc(meal.nome)} · ${esc(dayName)}</h2><button type="button" class="btn ghost sm" data-no>Fechar</button></div>
      ${fm ? `<p class="small"><strong>${esc(fm.base || meal.base_comum || "")}</strong>${fm.hora ? ` · às ${esc(fm.hora)}` : ""}</p>` : ""}
      <p class="small muted" style="margin:0 0 4px">Para quem cozinhas?</p>
      <div class="famwho" style="margin:0">${members.map((id) => `<label><input type="checkbox" data-for="${id}"${id === S.pid || inFam(id) ? " checked" : ""}> ${esc(id === S.pid ? "Eu" : nameOf(id))}${inFam(id) && id !== S.pid ? ' <span class="meta muted">(à mesa)</span>' : ""}</label>`).join("")}</div>
      <p class="hint" id="rcWho"></p>
      <ul class="items" id="rcRows"></ul>
      ${prep ? `<div class="prep">👩‍🍳 ${esc(prep)}</div>` : ""}
      ${(fm?.marmita || meal.marmita)?.preparar_em ? `<div class="marmita">🥡 Marmita: preparar ${esc((fm?.marmita || meal.marmita).preparar_em)}${(fm?.marmita || meal.marmita).conservacao ? ` · ${esc((fm?.marmita || meal.marmita).conservacao)}` : ""}${(fm?.marmita || meal.marmita).montagem ? `<br>${esc((fm?.marmita || meal.marmita).montagem)}` : ""}</div>` : ""}
      ${(meal.ordem || []).length ? `<div class="order">Ordem no prato: ${meal.ordem.map((o, i) => `${i ? '<span class="arrow">→</span> ' : ""}<b>${esc(o)}</b>`).join(" ")}</div>` : ""}
      <p class="hint">Marmitas: arrefecer em menos de 2 horas, frigorífico a 5 °C, no máximo 3 a 4 dias, reaquecer até fumegar e só uma vez; arroz no frio na hora e em 24 horas.</p>
    </div>`;
    wrap.addEventListener("change", build);
    wrap.addEventListener("click", (e) => { if (e.target === wrap || e.target.closest("[data-no]")) wrap.remove(); });
    document.body.appendChild(wrap); build();
  }
  /** Nota livre sobre uma refeição: entra na memória e nos próximos planos. */
  function openNote(dayName, idx) {
    const meal = S.plan?.plan?.dias?.find((d) => d.dia === dayName)?.refeicoes?.[idx]; if (!meal) return;
    const wrap = document.createElement("div"); wrap.className = "modal";
    const prev = S.prefs.filter((p) => p.voto === 0 && p.key === mealKey(meal)).slice(-3);
    wrap.innerHTML = `<div class="box" role="dialog" aria-modal="true">
      <div class="row between"><h2>💬 ${esc(meal.nome)} · ${esc(dayName)}</h2><button type="button" class="btn ghost sm" data-no>Fechar</button></div>
      <p class="small muted">O que mudavas? Fica na memória e entra nos próximos planos e nas trocas.</p>
      ${prev.length ? `<ul class="list">${prev.map((p) => `<li><span class="small">${esc(p.nota)}</span><button type="button" class="btn ghost sm" data-delpref="${p.id}">remover</button></li>`).join("")}</ul>` : ""}
      <textarea class="input" id="noteText" placeholder="Ex.: porção grande demais; faltou sal; prefiro peru em vez de frango; ficou pesado ao jantar" maxlength="240"></textarea>
      <div class="row"><button type="button" class="btn" data-no>Cancelar</button><button type="button" class="btn primary" data-save>Guardar nota</button></div>
    </div>`;
    wrap.addEventListener("click", async (e) => {
      if (e.target === wrap || e.target.closest("[data-no]")) { wrap.remove(); return; }
      if (e.target.closest("[data-save]")) {
        const nota = $("noteText").value.trim(); if (!nota) return;
        S.prefs = [...S.prefs, { id: uid(), at: new Date().toISOString(), key: mealKey(meal), dia: dayName, refeicao: meal.nome, itens: (meal.itens || []).map((i) => `${i.alimento} ${i.quantidade}`.trim()).join(", "), voto: 0, nota }];
        wrap.remove(); await savePrefs();
      }
    });
    document.body.appendChild(wrap); $("noteText").focus();
  }
...
Responde APENAS com JSON: {"lista":[{"corredor":"Talho","itens":[{"alimento":"peito de frango","quantidade":"1,2 kg","de":"grelhado, assado","despensa":false}]}]}`, { cache: false, modelTier: "quick", ...(signal ? { signal } : {}) });
  ...
  function adherenceSummary(n) {
    ...
    for (let i = n - 1; i >= 0; i--) {
      ...
      const items = Object.entries(day).map(([meal, v]) => `${meal}: ${v.status}${v.texto ? ` (${v.texto})` : ""}${v.status === "parcial" && v.itens ? ` (só ${v.itens.join(", ")})` : ""}`);
  ...
  function mealHtml(m, compact) {
    ...
    const tools = !compact && m._day !== undefined ? `<div class="row tools" style="gap:4px"><button type="button" class="btn ghost sm" data-editmeal="${esc(m._day)}|${m._idx}">✎ Editar</button><button type="button" class="btn ghost sm" data-swapmeal="${esc(m._day)}|${m._idx}" title="Pedir outra refeição para este momento">⟳ Trocar</button><button type="button" class="btn ghost sm" data-recipe="${esc(m._day)}|${m._idx}" title="Receita para cozinhar">🍳 Receita</button><span class="likes"><button type="button" class="btn ghost sm" data-like="${esc(m._day)}|${m._idx}|1" aria-pressed="${pref?.voto === 1}" title="Gostei">👍</button><button type="button" class="btn ghost sm" data-like="${esc(m._day)}|${m._idx}|-1" aria-pressed="${pref?.voto === -1}" title="Não gostei">👎</button><button type="button" class="btn ghost sm" data-note="${esc(m._day)}|${m._idx}" title="Nota para o plano" aria-pressed="${notesOf(m).length > 0}">💬${notesOf(m).length ? ` ${notesOf(m).length}` : ""}</button></span></div>` : "";
  ...
  function renderTodayMeals() {
    ...
    const one = (m, i) => { const st = adh[m.nome]?.status; const cls = st === "comi" ? "done" : st === "parcial" ? "partial" : st === "saltei" ? "skipped" : ""; const html = mealHtml(m, true).replace('<div class="meal">', `<div class="meal ${cls}">`); const extra = st === "outro" && adh[m.nome].texto ? `<div class="prep">↪ ${esc(adh[m.nome].texto)}${adh[m.nome].kcal != null ? ` <span class="muted small num">(≈ ${Math.round(adh[m.nome].kcal)} kcal · ${Math.round(adh[m.nome].proteina_g || 0)} g proteína, estimado)</span>` : S.sample ? ` <span class="muted small">(a estimar…)</span>` : ""}</div>` : st === "parcial" ? `<div class="prep">↪ comeu: ${esc((adh[m.nome].itens || []).join(", "))}</div>` : ""; const part = S.partialFor === m.nome ? partialHtml(m) : ""; const idx = (S.plan.plan.dias.find((x) => x.dia === todayDayName())?.refeicoes || []).indexOf(m); return html.replace(/<\/div>$/, `${extra}<div class="adhrow">${adherenceButtons(m, true)}<button type="button" class="btn sm" data-recipe="${esc(todayDayName())}|${idx}">🍳 Receita</button></div>${part}</div>`); };
  ...
  async function rebalanceRestOfDay(pend) {
    ...
    const isFixed = (m) => pend.fixas.some((n) => norm(n) === norm(m.nome)) || ["comi", "parcial", "outro", "saltei"].includes(eaten[m.nome]?.status);
    ...
    const soma = (ms) => ms.reduce((a, m) => { const x = consumedMacros(m, eaten[m.nome]) || mealMacros(m); return { kcal: a.kcal + x.kcal, proteina_g: a.proteina_g + x.proteina_g, fibra_g: a.fibra_g + x.fibra_g }; }, { kcal: 0, proteina_g: 0, fibra_g: 0 });
    ...
    const detalhe = (m) => { const a = eaten[m.nome]; const c = consumedMacros(m, a) || mealMacros(m); return { nome: m.nome, hora: m.hora, familia: !!m.familia, ...(a?.status === "outro" ? { comeu_em_vez: a.texto || "outra coisa", itens: a.itens_estimados || [] } : a?.status === "saltei" ? { saltou: true, itens: [] } : a?.status === "parcial" ? { comeu_so: a.itens || [] } : { itens: (m.itens || []).map((i) => `${i.alimento} ${i.quantidade}`) }), kcal: c.kcal, proteina_g: c.proteina_g, fibra_g: c.fibra_g }; };
...
Responde APENAS com JSON válido nesta forma: {"refeicoes":[${mealSchema}]}`;
  ...
  /**
   * Rede de segurança: quando a assistente responde só em texto (sem chamar ferramentas), um pedido
   * curto extrai o que ela afirmou ter decidido — refeição com quantidades, regra guardada, extra,
   * tensão — e a app aplica-o com as mesmas funções das ferramentas. Sugestões não contam.
   */
  async function applyFromAnswer(userText, answer) {
    const hoje = todayDayName(); const amanha = DAYS[(DAYS.indexOf(hoje) + 1) % 7];
    const prompt = `Lê a mensagem da pessoa e a resposta da assistente nutricional de uma app privada (português de Portugal). A app precisa de guardar o que a assistente DECIDIU ou AFIRMOU ter feito; nada do que é só sugestão, opção, hipótese ou pergunta.
Devolve JSON com "acoes" (lista, pode ser vazia), cada uma de um destes tipos:
- {"tipo":"refeicao","dia":"hoje|amanhã|segunda|…|domingo","refeicao":"Pequeno-almoço|Almoço|Lanche|Jantar|Ceia","itens":[{"alimento":"…","quantidade":"60 g","estado":"seco|cru|cozinhado|…"}],"preparacao":"…","kcal":490,"proteina_g":49,"fibra_g":4,"ja_comi":false} — só quando a assistente deu quantidades concretas para UMA refeição de um dia concreto que a pessoa vai fazer ou fez (ex.: "hoje ao almoço fica: macarrão 60 g, frango 130 g"). Num intervalo ("100-120 g") usa o meio ("110 g"). "ja_comi": true se a pessoa disse que já comeu.
- {"tipo":"regra","texto":"…"} — só quando a assistente disse que ficou guardada/registada uma regra duradoura (alimento ou grupo que não come, ordem por que come, horário fixo).
- {"tipo":"extra","descricao":"…","kcal_estimado":300,"proteina_g":5} — só quando a pessoa disse que comeu algo fora do plano e a assistente o registou/contou.
- {"tipo":"tensao","sistolica":135,"diastolica":85} — só quando a pessoa deu uma medição de tensão e a assistente a registou.
Hoje é ${hoje}; amanhã é ${amanha}. Se nada disto aconteceu, devolve {"acoes":[]}.
...
Responde APENAS com JSON válido: {"acoes":[…]}`;
    const res = await S.sample.json(prompt, { cache: false, modelTier: "default" });
    const acoes = Array.isArray(res?.acoes) ? res.acoes.slice(0, 4) : [];
    const feito = [];
    for (const a of acoes) {
      try {
        if (a?.tipo === "refeicao" && S.plan?.plan) {
          const d = norm(a.dia || ""); const dia = !d || d === "hoje" ? hoje : /amanh/.test(d) ? amanha : (DAYS.find((x) => norm(x) === d) || hoje);
          const r = await fixMealForDay({ ...a, dia });
          feito.push(`${r.refeicao.nome} de ${dia === hoje ? "hoje" : dia} no plano (${r.refeicao.kcal} kcal, ${r.refeicao.proteina_g} g de proteína)`);
        } else if (a?.tipo === "regra" && a.texto) {
          const r = await addRule(a.texto, "chat"); if (r && !r.duplicada) feito.push(`regra: ${r.texto}`);
        } else if (a?.tipo === "extra" && a.descricao && S.plan) {
          const r = await registerExtra(a); feito.push(`fora do plano: ${r.registado.descricao}`);
        } else if (a?.tipo === "tensao") {
          const r = await registerBP(a); feito.push(`tensão ${r.registado.sys}/${r.registado.dia}`);
        }
      } catch (e) { console.warn("aplicar", a, e); }
    }
    return feito;
  }
...
Formato: responde de forma direta e curta, em texto simples (sem títulos nem markdown pesado; podes usar listas com "-"). Quando ajustares o plano, diz claramente o que muda (ex: "amanhã ao jantar troco X por Y, ~150 g").`;
  ...
  function renderDiag() {
    ...
    el.textContent = [
      ...
      d.lastTools ? `última resposta do chat: ferramentas ${d.lastTools}` : "",
  ...
  async function sendMessageInner(text, files, log, ctl) {
    ...
    const usedTools = [];
    (toolList || []).forEach((t) => { const ex = t.execute; t.execute = async (...a) => { usedTools.push(t.name); return ex(...a); }; });
    ...
    try {
      ...
      try {
        ...
        if (e1?.code === "prompt_too_large") {
          ...
          S.diag.lastErr = `chat: ${e1.code} ${String(e1.message || "").slice(0, 160)} — repetido sem ferramentas`.replace(/\s+/g, " "); renderDiag();
    ...
    S.diag.lastTools = usedTools.length ? usedTools.join(", ") : "nenhuma"; renderDiag();
    // a assistente respondeu só em texto: a app guarda o que ela decidiu (refeição, regra, extra, tensão)
    if (!err && finalText && usedTools.length === 0 && S.sample && /\d+\s*(g|ml|kcal)|regist|guard|plano|regra|tens[ãa]o/i.test(finalText)) {
      const b1 = document.createElement("div"); b1.className = "msg assistant"; b1.innerHTML = `<span class="thinking">a guardar na app…</span>`; log.appendChild(b1); log.scrollTop = log.scrollHeight;
      try {
        const feito = await applyFromAnswer(text, finalText);
        if (feito.length) { pushChat({ role: "assistant", content: `✅ Guardei na app: ${feito.join("; ")}.`, at: new Date().toISOString() }); renderChat(); await saveChat(); }
      } catch (e) { if (e?.code !== "cancelled") { S.diag.lastErr = `aplicar resposta: ${e?.code || e?.message}`; renderDiag(); } }
      b1.remove();
    }
...
Responde APENAS com JSON válido: {"regras":["frase curta na 2.ª pessoa"],"gostei":["alimento ou prato"],"nao_gostei":["alimento ou prato"]}. Listas vazias quando não há nada novo.`;
  ...
  document.addEventListener("click", async (ev) => {
    const t = ev.target.closest("[data-pid],[data-view],[data-goto],[data-ml],[data-del],[data-range],[data-chip],[data-deltit],[data-delmed],[data-togglemed],[data-dellab],[data-day],[data-delbp],[data-deldoc],[data-delextra],[data-delrule],[data-delbody],[data-editbody],[data-editlab],[data-editbp],[data-sub],[data-attach],[data-editmeal],[data-sym],[data-adh],[data-like],[data-swapmeal],[data-regenday],[data-delpref],[data-recipe],[data-note],[data-psave],[data-pcancel],#retimeBtn,#rebalanceToday");
    if (!t) return;
    ...
    if (t.dataset.psave !== undefined) {
      const box = t.closest(".partial"); const meal = todayMeals().find((m) => m.nome === t.dataset.psave); if (!meal) return;
      const itens = [...box.querySelectorAll("[data-pitem]")].filter((c) => c.checked).map((c) => meal.itens[+c.dataset.pitem].alimento);
      S.partialFor = null; await setAdherence(meal.nome, itens.length === meal.itens.length ? "comi" : itens.length === 0 ? "saltei" : "parcial", null, itens); return;
    }
    if (t.dataset.pcancel !== undefined) { S.partialFor = null; renderHome(); return; }
    if (t.dataset.recipe) { const [d, i] = t.dataset.recipe.split("|"); openRecipe(d, +i); return; }
    if (t.dataset.note) { const [d, i] = t.dataset.note.split("|"); openNote(d, +i); return; }
    if (t.dataset.adh) {
      const name = t.dataset.adh, st = t.dataset.st;
      if (st === "parcial") { S.partialFor = S.partialFor === name ? null : name; renderHome(); return; }
    ...
    if (t.id === "rebalanceToday") { await rebalanceTodayFromHome(); return; }
  ...
  (async () => {
    ...
    if (sample) {
      ...
      estimateOthers().catch((e) => console.warn("estimar", e));
